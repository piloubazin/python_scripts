...
#                                    output_dir=out_dir)
    ...
    print("4. Vasculature from R2*")
    
    r2strip = nibabel.Nifti1Image(nighres.io.load_volume(qr2['r2s']).get_data()*nighres.io.load_volume(skull['brain_mask']).get_data(), nighres.io.load_volume(qr2['r2s']).affine, nighres.io.load_volume(qr2['r2s']).header)
    
    vasc = nighres.filtering.multiscale_vessel_filter(r2strip, structure_intensity='bright', filterType = 'RRF',
    ...
    print("5. Vasculature from R1")
    
    #s0strip = nibabel.Nifti1Image(nighres.io.load_volume(qr2['s0']).get_data()*nighres.io.load_volume(skull['brain_mask']).get_data(), nighres.io.load_volume(qr2['s0']).affine, nighres.io.load_volume(qr2['s0']).header)
    r1strip = nibabel.Nifti1Image(nighres.io.load_volume(qr1['r1']).get_data()*nighres.io.load_volume(skull['brain_mask']).get_data(), nighres.io.load_volume(qr1['r1']).affine, nighres.io.load_volume(qr1['r1']).header)
    
    vasc = nighres.filtering.multiscale_vessel_filter(r1strip, structure_intensity='bright', filterType = 'RRF',
                                    propagationtype = 'diffusion', threshold = 0.5,
                                    factor = 0.5,max_diff = 0.001,max_itr = 100,
                                    scale_step = 1.0,
                                    scales = 3,
