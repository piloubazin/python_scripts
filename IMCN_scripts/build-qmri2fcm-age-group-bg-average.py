...
decades = [decade1880,decade6180,decade4160,decade1840]
declb = ['18to80','61to80','41to60','18to40']
...
for dec,decade in enumerate(decades):
    ...
    for subject in decade: 
        ...
        print("adding: "+parcellation+" to bg atlas")
        if os.path.exists(parcellation) is False: print("error: whole brain doesn't exist")
            ...
            data = img.get_data()
            struct = nb.Nifti1Image(data==0,img.affine,img.header)
    ...
    avg_name = 'ahead-qmri2fcm_avg-background_decade-'+declb[dec]+'_n'+str(nsubjects)+'.nii.gz'
    ...
    std_name = 'ahead-qmri2fcm_std-background_decade-'+declb[dec]+'_n'+str(nsubjects)+'.nii.gz'
