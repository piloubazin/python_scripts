...
for subject in range(0,maxsubject): 
    ...
    if os.path.exists(whole_brain) is False: print("error: whole brain doesn't exist")
        ...
        data = nr.io.load_volume(whole_brain).get_fdata()
        ...
        for dz in range(subsample) :
            ...
            for subject in range(0,maxsubject) : 
                ...
                if os.path.exists(whole_brain) is False: print("error: whole brain doesn't exist")
                    ...
                    histos[:,:,:,subnum] = nr.io.load_volume(mapped['result']).get_fdata()[xmin:xmax,ymin:ymax,zmin:zmax]/scaling[subnum]
                    subnum = subnum+1
...
med_name = 'ahead_qmri2_mni09b_med_pdmap_n'+str(nsubjects)+'.nii.gz'
save_volume(med_name, med_img)

iqr_img = nb.Nifti1Image(iqr, affine, header)
iqr_name = 'ahead_qmri2_mni09b_iqr_pdmap_n'+str(nsubjects)+'.nii.gz'
