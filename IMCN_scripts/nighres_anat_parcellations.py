...
#subjects = ['sub-002','sub-003','sub-004','sub-005','sub-006','sub-007','sub-008','sub-009',
#            'sub-010','sub-011','sub-012','sub-013','sub-014','sub-015','sub-016','sub-017',
#            'sub-018','sub-019','sub-020','sub-021']
#subjects = ['sub-023','sub-024','sub-026']
...
# here we assume the qMRI pipeline has been run

for subject in subjects:

    main_dir='/home/Public/trondheim/'
    
    in_dir=main_dir+'derivatives/nighres/'+subject+'/'+session+'/qmri/'
    proc_dir=main_dir+'processing/'+subject+'/'
    sub_dir=main_dir+'derivatives/nighres/'+subject+'/'+session+'/subcortex/'
    ctx_dir=main_dir+'derivatives/nighres/'+subject+'/'+session+'/cortex/'
    vsl_dir=main_dir+'derivatives/nighres/'+subject+'/'+session+'/vessels/'
    tis_dir=main_dir+'derivatives/nighres/'+subject+'/'+session+'/tissues/'
    
    # input names
    qr1_file = in_dir+subject+'_'+session+'_acq-mp2rage_mod-r1hz_orient-std_brain.nii.gz'
    qr2s_file = in_dir+subject+'_'+session+'_acq-gre_mod-r2hz_orient-std_brain.nii.gz'
    qsm_file = in_dir+subject+'_'+session+'_acq-gre_mod-qsm_orient-std_brain.nii.gz'
    qpd_file = in_dir+subject+'_'+session+'_acq-mp2rage_mod-qpd_orient-std_brain.nii.gz'
    ...
    tis_map_file = tis_dir+subject+'_'+session+'_r1tim'+tis_map
    ...
    tis_map_file = tis_dir+subject+'_'+session+'_r2stim'+tis_map
    if (not os.path.exists(tis_map_file)):
    ...
    tis_map_file = tis_dir+subject+'_'+session+'_qsmtim'+tis_map
    if (not os.path.exists(tis_map_file)):
    ...
    tis_map_file = tis_dir+subject+'_'+session+'_qpdtim'+tis_map
    if (not os.path.exists(tis_map_file)):
    ...
    sub_label_file = sub_dir+subject+'_'+session+sub_label
    sub_proba_file = sub_dir+subject+'_'+session+sub_proba
...
#                                ignore_affine=True, 
#                                save_data=True, file_name=subject+'_'+session+'_reg2ahead',
#                                output_dir=proc_dir)
    ...
    mgdm_label_file = ctx_dir+subject+'_'+session+mgdm_label
    mgdm_proba_file = ctx_dir+subject+'_'+session+mgdm_proba
    
    if (not os.path.exists(mgdm_label_file) or not os.path.exists(mgdm_proba_file)):
        qt1_file = proc_dir+subject+'_'+session+'_acq-mp2rage_mod-t1msnob1_orient-std_brain.nii.gz'
        ...
        mgdm = nighres.brain.mgdm_segmentation(contrast_image1=qr1_file,
                                                ...
                                                save_data=True, file_name=subject+'_'+session,
                                                output_dir=proc_dir)
        ...
        mgdm_mems_file = proc_dir+subject+'_'+session+'_mgdm-mems.nii.gz'
        mgdm_lbls_file = proc_dir+subject+'_'+session+'_mgdm-lbls.nii.gz'
    ...
    for idx,region in enumerate(regions):
        ctx_label_file = ctx_dir+subject+'_'+session+'_'+rgns[idx]+ctx_label
        ctx_gwb_file = ctx_dir+subject+'_'+session+'_'+rgns[idx]+ctx_gwb
        ctx_cgb_file = ctx_dir+subject+'_'+session+'_'+rgns[idx]+ctx_cgb
        ...
        if (not os.path.exists(ctx_label_file) or not os.path.exists(ctx_gwb_file) or not os.path.exists(ctx_cgb_file)):
            ...
            cortex = nighres.brain.extract_brain_region(segmentation=mgdm['segmentation'],
                                                        ...
                                                        save_data=True,
                                                        output_dir=proc_dir, 
                                                        file_name=subject+'_'+session+'_'+rgns[idx])
    ...
    vsl_pv_file = vsl_dir+subject+'_'+session+'_r2vsl'+vsl_pv
    vsl_dia_file = vsl_dir+subject+'_'+session+'_r2vsl'+vsl_dia
    
    art_pv_file = vsl_dir+subject+'_'+session+'_r1art'+vsl_pv
    art_dia_file = vsl_dir+subject+'_'+session+'_r1art'+vsl_dia
    
    ven_pv_file = vsl_dir+subject+'_'+session+'_qsmven'+vsl_pv
    ven_dia_file = vsl_dir+subject+'_'+session+'_qsmven'+vsl_dia
